...
public class SlidingContainer extends RelativeLayout {
    ...
    private static final String TAG = "SlidingContainer";
    /**
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        translate(event);
//        Logger.i("Sliding是否拦截:"+ntercept+"---isIntercept:"+isIntercept);
//        return super.onInterceptTouchEvent(event);
//        Logger.i("Sliding是否拦截:"+isIntercept);
        return isIntercept;
    }
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (Math.abs(touchDelta) > slideThreshold) {
                    ...
                    if (isMoveY){
                        float scale = translated/stopValues.get(0);
                        slideListener.onScaleY(scale);
//                        Log.i(TAG, "translated:"+translated);
    ...
    private void animate(float amount) {
        Log.i(TAG, "滑动关闭或打开animate: "+amount);
        animate(amount, duration, new LinearInterpolator());
    }

    public void toBottom(){
        stopValueIndex = 0;
        animate(stopValues.get(stopValueIndex), duration, new LinearInterpolator());
        startYCoordinate = -1;
        touchDelta = 0;
    }
    ...
    private void animate(final float amount, int duration, Interpolator interpolator) {
        ...
        oa.addListener(new CustomAnimationListener() {
            ...
            public void onAnimationEnd(Animator animator) {
                slideListener.onSlideEndByAnimator(amount);
                onSettled(stopValueIndex);
            }
        });
        oa.start();
