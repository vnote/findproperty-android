...
public class HorizontalItemSelector extends View {
    ...
    public HorizontalItemSelector(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.horizontalSelector);
    }

    public HorizontalItemSelector(Context context, AttributeSet attrs, int defStyle) {
        ...
        try {
            textColor = a.getColorStateList(R.styleable.HorizontalItemSelector_android_textColor);
            if (textColor == null) {
                textColor = ColorStateList.valueOf(0xFF000000);
            }

            values = a.getTextArray(R.styleable.HorizontalItemSelector_values);
            ellipsize = a.getInt(R.styleable.HorizontalItemSelector_android_ellipsize, ellipsize);
            marqueeRepeatLimit = a.getInt(R.styleable.HorizontalItemSelector_android_marqueeRepeatLimit, marqueeRepeatLimit);
            dividerSize = a.getDimension(R.styleable.HorizontalItemSelector_dividerSize, dividerSize);
            sideItems = a.getInt(R.styleable.HorizontalItemSelector_sideItems, sideItems);

            float textSize = a.getDimension(R.styleable.HorizontalItemSelector_android_textSize, -1);
            if(textSize > -1) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int w = getWidth();
        int h = getHeight();
        Rect clientRect1 = new Rect(w/2,0,w,h);
        Rect clientRect2 = new Rect(0,0,w/2,h);

        LinearGradient shader = new LinearGradient(w/2,h/2,w,h/2,new int[]{0xffffffff,0x77ffffff,0x00ffffff},new float[]{0,0.5f,1}, Shader.TileMode.MIRROR);
        LinearGradient shader1 = new LinearGradient(w/2,h/2,0,h/2,new int[]{0xffffffff,0x77ffffff,0x00ffffff},new float[]{0,0.5f,1}, Shader.TileMode.MIRROR);

        Paint paint = new Paint();
        paint.setShader(shader);
        canvas.drawRect(clientRect1,paint);
        paint.setShader(shader1);
        canvas.drawRect(clientRect2,paint);

        Paint paint_line = new Paint();
        paint_line.setColor(getResources().getColor(R.color.appBaseColor));
        RectF line1 = new RectF(w/2-0.5f,0,w/2+0.5f,10);
        canvas.drawRect(line1,paint_line);
        RectF line2 = new RectF(w/2-0.5f,h,w/2+0.5f,h-10);
        canvas.drawRect(line2,paint_line);
...
//            postInvalidateOnAnimation(); // TODO
    ...
    private static final class Marquee extends Handler {
        ...
        private final WeakReference<HorizontalItemSelector> mView;
        ...
        Marquee(HorizontalItemSelector v, Layout l, boolean rtl) {
            ...
            mView = new WeakReference<HorizontalItemSelector>(v);
        ...
        void tick() {
            ...
            final HorizontalItemSelector view = mView.get();
    ...
    private static class PickerTouchHelper extends ExploreByTouchHelper {
        ...
        private HorizontalItemSelector mPicker;

        public PickerTouchHelper(HorizontalItemSelector picker) {
            super(picker);
            mPicker = picker;
        }
...
//    <com.cetnaline.findproperty.widgets.HorizontalItemSelector
//    android:id="@+id/picker"
//    android:layout_width="match_parent"
//    android:layout_height="wrap_content"
//    android:textColor="@drawable/horizontal_item_color"
