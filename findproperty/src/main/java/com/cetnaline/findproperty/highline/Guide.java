...
public class Guide implements View.OnKeyListener, View.OnClickListener {
  ...
  private MaskView onCreateView(Activity activity) {
    ...
    if (mConfiguration.mTargetView != null) {
      maskView.setTargetRect(Common.getViewAbsRect(mConfiguration.mTargetView, parentX, parentY, activity));
    } else {
      // Gets the target view's abs rect
      View target = activity.findViewById(mConfiguration.mTargetViewId);
      if (target != null) {
        maskView.setTargetRect(Common.getViewAbsRect(target, parentX, parentY, activity));
      }
    }
    ...
    if (fulling != null) {
      maskView.setFullingRect(Common.getViewAbsRect(fulling, parentX, parentY, activity));
    }
