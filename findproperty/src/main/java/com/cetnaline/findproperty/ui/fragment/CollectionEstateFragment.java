...
import com.cetnaline.findproperty.ui.listadapter.CollectionEstateAdapter;
...
public class CollectionEstateFragment extends BaseFragment {
    ...
    private CollectionEstateAdapter adapter;
    ...
    protected void init() {
        ...
        datas = new ArrayList<>();
        adapter = new CollectionEstateAdapter(getActivity(),datas,R.layout.item_estate);
        ...
        data_list.setOnMenuItemClickListener(new SwipeMenuListView.OnMenuItemClickListener() {
            ...
            public boolean onMenuItemClick(int position, SwipeMenu menu, int index) {
                switch (index) {
                    case 0:
                        String code = datas.get(position).getEstateCode();
                        long collectId=-1;
                        for (CollectionBean bean: collections) {
                            if (bean.getCollectValue().equals(code)) {
                ...
                put("Count","1000");
                    ...
                    public void call(BaseResult<CollectionBean> collectionBeanBaseResult) {
                        if (collectionBeanBaseResult.Result != null) {
                            ...
                            mCompositeSubscription.add(ApiRequest.getEstatesByCodeList(ids.substring(0, ids.length() - 1))
                                    .subscribe(result -> {
                                        cancelLoadingDialog();
                                        if ((result.getResult() == null || result.getResult().size() == 0) && isAllLoaded == false) {
                                            showMessage(true);
                                        }
                                        List<EstateBo> list = result.getResult();
                                        datas.addAll(list);
                                        adapter.notifyDataSetChanged();
                                        current_page++;
                                        showMessage(false);
...
//        if (event.type.equals(CancelCollectionEvent.ESTATE)) {
//            if (!event.saveCollection && event.value != null){
//                for (EstateBo bean : datas) {
//                    if (bean.getEstateCode().equals(event.value)) {
//                        datas.remove(bean);
